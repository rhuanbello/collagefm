...
// Function to validate grid size
...
export default async function CollagePage({
  ...
  const period = typeof searchParams.period === 'string' ? searchParams.period : 'overall';
  const type = typeof searchParams.type === 'string' && allowedTypes.includes(searchParams.type) ? searchParams.type : 'albums';
  const gridSizeParam = typeof searchParams.gridSize === 'string' ? searchParams.gridSize : '3x3';
  const gridSize = isValidGridSize(gridSizeParam) ? gridSizeParam : '3x3';

  // If no username is provided, show an error
  if (!username) {
    return (
        ...
        <Suspense fallback={
          <div className="flex flex-col items-center justify-center min-h-[50vh] relative z-10">
            <div className="rounded-xl backdrop-blur-md bg-white/50 dark:bg-gray-900/50 p-8 flex flex-col items-center">
              ...
              <p className="mt-6 text-lg text-gray-700 dark:text-gray-300 font-medium">Loading collage data...</p>
            </div>
          </div>
        }>
      ...
      <main className="min-h-screen flex flex-col items-center justify-center p-8 relative overflow-hidden">
        ...
        <div className="backdrop-blur-md bg-white/60 dark:bg-gray-900/60 rounded-2xl p-8 shadow-2xl border border-white/30 dark:border-gray-800/30 max-w-md w-full relative z-10">
          <div className="text-center space-y-4">
            ...
            <p className="text-gray-700 dark:text-gray-300 text-lg">
              Failed to load collage data. Please try again.
            </p>
            <div className="pt-4">
              <Link 
                href="/"
